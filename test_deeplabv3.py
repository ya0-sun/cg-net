...
# ***************************** path *******************************
weight_path = 'weights/cgnet_deeplabv3.h5' # your weight
...
# ************************ initialize model ************************
model = deeplabv3_SPADE(patch_size, nb_channel, nb_classes, downsample_ratio, pre_train)
model.summary()
...
# ####################### produce outputs ##########################
y_pred = model.predict([np.concatenate([X_val1, X_val2], -1), X_val2], batch_size)
y_pred = np.uint8(np.round(y_pred))

with h5py.File('y_pred_both_d8_prob_insar_deeplabv3.mat', 'w') as f:
    f['y_pred'] = y_pred
